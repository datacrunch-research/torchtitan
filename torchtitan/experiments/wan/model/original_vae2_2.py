...
def main():
    """
    Example code to test Wan2_2_VAE model behavior.
    This matches the test code in wan_vae.py for comparison.
    """
    # Initialize logging
    ...
    # Load Wan2_2_VAE model
    logger.info("Loading Wan2_2_VAE")
    vae = Wan2_2_VAE(
    ...
    for batch in dataloader:
        ...
        # Wan2_2_VAE.encode expects a list of tensors, each of shape (C, T, H, W)
        # Convert batched tensor (B, T, C, H, W) to list of (C, T, H, W) tensors
        B, T, C, H, W = input_video.shape
        video_list = []
        for b in range(B):
            # Convert (T, C, H, W) to (C, T, H, W) for each video in batch
            video_list.append(input_video[b].transpose(0, 1))  # (C, T, H, W)
        
        # Encode videos
        latents = vae.encode(video_list)
        
        # Decode latents
        pred_video_list = vae.decode(latents)
        
        # Convert list of (C, T, H, W) back to batched tensor (B, T, C, H, W)
        pred_video_batch = []
        for pred_video in pred_video_list:
            # pred_video is (C, T, H, W), convert to (T, C, H, W)
            pred_video_batch.append(pred_video.transpose(0, 1))
        pred_video = torch.stack(pred_video_batch, dim=0)  # (B, T, C, H, W)
        
        # Compute PSNR across frames
        # input_video: (B, T, C, H, W), pred_video: (B, T, C, H, W)
        # Ensure both are on same device and dtype for PSNR computation
        input_video_float = input_video.float()
        pred_video_float = pred_video.float()
