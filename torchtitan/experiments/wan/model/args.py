...
class WanModelArgs(BaseModelArgs):
    ...
    def update_from_config(self, job_config: JobConfig, **kwargs) -> None:
        ...
        if (
            hasattr(job_config.model, "num_layers")
            and job_config.model.num_layers is not None
        ):
    ...
    def get_nparams_and_flops(self, model: nn.Module, seq_len: int) -> tuple[int, int]:
        ...
        logger.info(
            f"Wan model parameters: {nparams:,}, FLOPS per token: {flops_per_token:,.0f}, "
            f"head_dim: {self.hidden_dim // self.num_heads}, num_layers: {self.num_layers}, "
            f"num_heads: {self.num_heads}, seq_len: {actual_seq_len} "
            f"(computed from video dimensions, config had {seq_len})"
        )
...
def dit_flops_per_video_token(
    ...
    logger.info(
        f"FLOPS per token: {flops_per_token:,.0f}, num_layers: {num_layers}, "
        f"flops_per_layer_train: {flops_per_layer_train}"
    )
