...
class FlowMatchScheduler:
    ...
    def add_noise_batch(self, original_samples, noise, timesteps_idxs):
        sigma = self.sigmas[timesteps_idxs].to(original_samples.device)
        # Reshape sigma to broadcast correctly with the sample tensors
        # sigma shape: [batch_size] -> [batch_size, 1, 1, 1, 1]
        sigma = sigma.view(-1, 1, 1, 1, 1)
        sample = (1 - sigma) * original_samples + sigma * noise
        return sample

    def training_target(self, sample, noise, timestep=None):
        target = noise - sample
        return target
    ...
    def training_weight_batch(self, timesteps_idxs):
        weights = self.linear_timesteps_weights[timesteps_idxs]
        return weights
